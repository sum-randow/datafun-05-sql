...
def main() -> None:
    ...
    try:
        ...
        db_store_count = cursor.fetchone()[0]
        csv_store_count = (
            sum(1 for _ in Path.open(STORE_CSV)) - 1
        )  # subtract header row
        LOG.info(f"Store rows — DB: {db_store_count}, CSV: {csv_store_count}")
        assert db_store_count == csv_store_count, "Mismatch in store row count!"

        cursor.execute("SELECT COUNT(*) FROM sale")
        db_sale_count = cursor.fetchone()[0]
        csv_sale_count = sum(1 for _ in Path.open(SALE_CSV)) - 1
        LOG.info(f"Sale rows  — DB: {db_sale_count}, CSV: {csv_sale_count}")
        assert db_sale_count == csv_sale_count, "Mismatch in sale row count!"
        ...
        run_sql_query(con, SQL_DIR / "randow_retail_query_top_products.sql")
